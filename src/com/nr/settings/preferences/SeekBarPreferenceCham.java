...
public class SeekBarPreferenceCham extends Preference implements SeekBar.OnSeekBarChangeListener {
    ...
    private static final String NREXTRAS = "http://schemas.android.com/apk/res/com.nr.settings";
    ...
    private void setValuesFromXml(AttributeSet attrs, Context context) {
        ...
        mUnitsLeft = getAttributeStringValue(attrs, NREXTRAS, "unitsLeft", "");
        mUnitsRight = getAttributeStringValue(attrs, NREXTRAS, "unitsRight", "");
        Integer idR = a.getResourceId(R.styleable.CustomSeekBarPreference_unitsRight, 0);
        if (idR > 0) {
            mUnitsRight = context.getResources().getString(idR);
        }
        Integer idL = a.getResourceId(R.styleable.CustomSeekBarPreference_unitsLeft, 0);
        if (idL > 0) {
            mUnitsLeft = context.getResources().getString(idL);
        }
        try {
            String newInterval = attrs.getAttributeValue(NREXTRAS, "interval");
            if(newInterval != null)
        ...
        mThumbDefaultValueColor = a.getColor(
                R.styleable.CustomSeekBarPreference_thumb_default_value_color, 0xff000000);
        a.recycle();
    }
